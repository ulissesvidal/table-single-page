 ...
 * @since       12.1
 */
class JDatabaseDriverMssql2000 extends JDatabaseDriver
{
	/**
...
//			'ReturnDatesAsStrings' => true);
		...
		{
			throw new RuntimeException('PHP extension mssql_connect is not available.');
		}

		// Attempt to connect to the server.
		if (!($this->connection = @ @mssql_connect( $this->options['host'], $this->options['user'], $this->options['password'], true )))
		{
			throw new RuntimeException('Database mssql_connect failed');
		}
	 ...
	 *
	 * @return  JDatabaseDriverMssql  Returns this object to support chaining.
	 *
	 * @since   12.1
	 */
...
//		$this->connect();
//
//		$version = mssql_server_info($this->connection);
//		return $version['SQLServerVersion'];
	...
	public function loadResult()
	{
		...
		if ($row = mssql_fetch_array($cursor, MSSQL_NUM))
		{
			$ret = $row[0];
		}
		...
		if (!$this->cursor)
		{
			...
			if (!$this->connected())
			{
				...
				catch (RuntimeException $e)
				{
					...
					$this->errorNum =0 ; //$errors[0]['SQLSTATE'];
...
//		if (!mssql_query($this->connection, 'USE ' . $database, null, array('scrollable' => SQLSRV_CURSOR_STATIC)))
//		{
...
//		}
		...
		if (!mssql_select_db( $database, $this->connection ))
		{
			...
			{
				throw new RuntimeException(JText::_('Could not connect to database'));
			}
		}
	...
	{
		return mssql_fetch_array($cursor ? $cursor : $this->cursor, MSSQL_NUM);
	}
	...
	{
		return mssql_fetch_array($cursor ? $cursor : $this->cursor, MSSQL_ASSOC);
	}
